        ...
        protected void save0_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < Context.Request.Files.Count; i++)
                {
                    ...
                    string CADENA = string.Format("insert into DOCUMENT_SHOP values('" + id + "','" + GridView1.SelectedRow.Cells[8].Text + "','" + "http://canezpowerdivision.com/canezpdf/" + nombre + "',GETDATE(),'" + GetTime() + "','" + "service" + "')");
                    DataSet DS = CLASS_CONECTAR.CONECTAR(CADENA);
                }
            }
            catch (Exception ERROR)
