...
namespace CanezPower
{
    public partial class INVOICES : System.Web.UI.Page
    {
        ...
        void EDIT_ENVOICES()
        {
            ...
            try
            {
                string COMANDO = string.Format("EXEC  EDIT_INVOICES '"+id+"','" + "GETDATE()" + "','" + DROPCLIENT.Text + "','" + TXTINV.Text + "','" + TXTDESCRIPTION.Text + "','" + TXTTOTAL.Text + "','"+CBSTATUS.Text+"','" + GetTime() + "'");
                DataSet DS = CLASS_CONECTAR.CONECTAR(COMANDO);
        ...
        protected void save0_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string COMANDO = string.Format("EXEC  NEW_INVOICES'" + "GETDATE()" + "','" + DROPCLIENT.Text + "','" + TXTINV.Text + "','" + TXTDESCRIPTION.Text + "','" + TXTTOTAL.Text + "','" + CBSTATUS.Text + "','"+nombre+"','" + GetTime() + "'");
                    DataSet DS = CLASS_CONECTAR.CONECTAR(COMANDO);
                    LBLESTADO.Text = "SE REALIZO CORRECTAMENTE";
                    LIMPIAR();
                    BTNGUARDAR.Enabled = true;
