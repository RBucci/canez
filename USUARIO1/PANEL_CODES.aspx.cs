...
namespace CanezPower.USUARIO1
{
    public partial class PANEL_CODES : System.Web.UI.Page
    {
        ...
        void NE_CLIENT()
        {
            try
            {
                string COMANDO = string.Format("exec new_panelcodes '" + TXTCODE.Text + "','" + TXTFMI.Text + "','" + DROPENGINE.SelectedValue + "','" + TXTDESCRIPTION.Text + "','" + GetTime() + "'");
        ...
        protected void TXTEDIT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string COMANDO = string.Format("exec edit_panelcodes '"+id+"','" + TXTCODE.Text + "','" + TXTFMI.Text + "','" + DROPENGINE.SelectedValue + "','" + TXTDESCRIPTION.Text + "','" + GetTime() + "'");
                DataSet DS = CLASS_CONECTAR.CONECTAR(COMANDO);
                LBLESTADO.Text = "SE REALIZO CORRECTAMENTE";
