    ...
    public partial class LOG : System.Web.UI.Page
    {
        ...
        protected void TXTSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string COMANDO = string.Format("SELECT DISTINCT        TBLOG.id, TBLOG.Date, TBLOG.Site, TBLOG.Genset, Genset_Serial AS [Genset Serial], Reference, honning_Hours AS [Running Hours], Notas AS [Description],CASE WHEN (DOCUMENT_SHOP.id_genset <> '" + "" + "' AND DOCUMENT_SHOP.tipo = 'log') THEN '1' ELSE '0' END AS Attachment, User_Login as [User] FROM            edgar2211.TBLOG  LEFT OUTER JOIN DOCUMENT_SHOP on edgar2211.TBLOG.GENSET = edgar2211.DOCUMENT_SHOP.GENSET and TBLOG.id = DOCUMENT_SHOP.id_genset   where TBLOG." + DropDownList1.Text + " like '%" + TXTREFENCE1.Text + "%'   order by TBLOG.id desc");
                DataSet DS = CLASS_CONECTAR.CONECTAR(COMANDO);
        ...
        protected void TXTSearch0_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string COMANDO = string.Format("SELECT DISTINCT       TBLOG.id, Date, Site, TBLOG.Genset, Genset_Serial AS [Genset Serial], Reference, honning_Hours AS [Running Hours], Notas AS [Description],CASE WHEN (DOCUMENT_SHOP.id_genset <> '" + "" + "' AND DOCUMENT_SHOP.tipo = 'log') THEN '1' ELSE '0' END AS Attachment, User_Login as [User] FROM            edgar2211.TBLOG  LEFT OUTER JOIN DOCUMENT_SHOP on edgar2211.TBLOG.GENSET = edgar2211.DOCUMENT_SHOP.GENSET and   TBLOG.id = DOCUMENT_SHOP.id_genset   where TBLOG." + DropDownList1.Text + " like '%" + TXTREFENCE1.Text + "%'  and (TBLOG.Date >= '" + TXTDESDE.Text + "') and (TBLOG.Date <= '" + TXTHASTA.Text + "')   order by TBLOG.id desc");
                    DataSet DS = CLASS_CONECTAR.CONECTAR(COMANDO);
                    if (DS.Tables[0].Rows.Count <= 0)
                    {
                        Response.Write("<script>alert('This item was not found');</script>");
        ...
        protected void save0_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string CADENA = string.Format("insert into DOCUMENT_SHOP values('" + id + "','" + GridView1.SelectedRow.Cells[5].Text + "','" + nombre + "',GETDATE(),'" + GetTime() + "','" + "log" + "')");
