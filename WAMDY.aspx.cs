    ...
    public partial class WAMDY : System.Web.UI.Page
    {
        ...
        void MOSTRAR()
        {
            ...
            try
            {
                ...
                string COMANDO = string.Format(" SELECT        ID, DATEIN AS [DATE IN], CLIENT, SITE, MOTOR_REFF AS [MOTOR REFF], CAPACITY, MOTOR_SERIAL AS [MOTOR SERIAL #], JOB_DESCRIPTION AS [JOB DESCRIPTION], DATEOUT AS [DATE OUT], INVOICE,                         '$'+ TOTAL+'.00' as  [WANDY TOTAL], STATUS, WARRANTY_TAG AS [WARRANTY TAG], CPD_INV AS [CPD INV], '$'+CPD_TOTAL+'.00' AS [CPD TOTAL],case when( (CONVERT(INT,CPD_TOTAL)-CONVERT(INT,TOTAL)))<=0THEN 0 ELSE (CONVERT(INT,CPD_TOTAL)-CONVERT(INT,TOTAL)) END AS [CPD PROFIT], CPD_INV_STATUS AS [CPD INV STATUS], COMMENT, USER1 FROM            edgar2211.WANDY ORDER BY ID DESC");
