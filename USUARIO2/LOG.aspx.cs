        ...
        protected void save0_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < Context.Request.Files.Count; i++)
                {
                    ...
                    string CADENA = string.Format("insert into DOCUMENT_SHOP values('" + id + "','" + GridView1.SelectedRow.Cells[5].Text + "','" + "http://canezpowerdivision.com/USUARIO2/canezpdf/" + nombre + "',GETDATE(),'" + GetTime() + "','" + "log" + "')");
                    DataSet DS = CLASS_CONECTAR.CONECTAR(CADENA);
                }
                Response.Redirect("~/USUARIO2/LOG.aspx");
