        ...
        protected void save0_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < Context.Request.Files.Count; i++)
                {
                    ...
                    string CADENA = string.Format("insert into DOCUMENT_SHOP values('" + id + "','" + GridView1.SelectedRow.Cells[4].Text + "','" + "http://canezpowerdivision.com/canezpdf/" + nombre + "',GETDATE(),'" + GetTime() + "','" + "sp" + "')");
                    DataSet DS = CLASS_CONECTAR.CONECTAR(CADENA);
                }
            }
            catch (Exception ERROR)
